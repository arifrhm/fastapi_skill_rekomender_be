        ...
        for skill in skills:
            await session.execute(
                insert(user_skills).values(
                    user_id=new_user.user_id, skill_id=skill.skill_id
                )
            )
        await session.commit()
